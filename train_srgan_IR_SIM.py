...
def validate(model, valid_dataloader, psnr_criterion, ssim_criterion, similaity_criterion, epoch, writer) -> float:
    ...
    with torch.no_grad():
        ...
        if epoch % 10 == 0:
            ...
            if epoch == 0:
                # Write once
                writer.add_image("Valid/Test_Input_IR",lr.squeeze(0),epoch + 1 )
                writer.add_image("Valid/Test_GroundTruth",high_img.squeeze(0),epoch + 1 )
            # Write everytime
            writer.add_image("Valid/Test_Output",sr.squeeze(0),epoch + 1 )
